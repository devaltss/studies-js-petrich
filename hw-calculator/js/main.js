...
optionalExpensesBtn.addEventListener('click', function() {
  for (let i = 0; i < optionalExpensesItem.length; i++) {
    let opt = optionalExpensesItem[i].value;         //prompt("Статья необязательных расходов?", "");
...
incomeItem.addEventListener('input', function() {
  let items = incomeItem.value;                //prompt('Что принесёт дополнительный доход? (Перечислите через запятую)', '');
